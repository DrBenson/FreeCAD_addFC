...
update_locale() {
	...
	if [ "$u" == "" ]; then
		...
		eval $LUPDATE "$FILES" -source-language en_US -target-language "${locale//-/_}" \
			-ts "${WB}_${locale,,}.ts"
		if [ "X${locale}" == "Xzh-TW" ]; then
			lconvert-qt6 -locations absolute -source-language en_US -target-language zh-TW -sort-contexts --verbose -no-obsolete -i addFC.ts addFC_zh-tw.ts   /media/ERP/sources/FreeCAD/Local_Mod/addFC_zh-TW.ts -o addFC_zh-TW.ts
		fi
	fi
}
...
LUPDATE=lupdate # from Qt6
# LUPDATE=lupdate                  # from Qt5
LRELEASE=lrelease # from Qt6
